    ...
    <>
      ...
      >
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
          ...
          <button type="button" onClick={closeCart} className="btn-ghost px-2.5 py-2" aria-label="Close cart">
            <X className="size-5" />
          </button>
        </div>
        ...
        {lines.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6 text-center">
            ...
            <p className="text-brand-200">Nothing here yet.</p>
              ...
              <div className="border-b border-white/10 px-5 py-3">
                <p className="flex items-center gap-2 text-xs text-brand-200">
                  ...
                  {remainingForFreeShipping > 0 ? (
                    <>
                      Add <strong className="text-white">{formatPrice(remainingForFreeShipping)}</strong>{' '}
                      for free shipping
                    </>
                  ) : (
                    <span className="font-semibold text-neon-400">Free shipping unlocked</span>
                  )}
                </p>
              ...
              {lines.map((line) => (
                <li key={line.productId} className="flex gap-3 py-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 text-xs text-brand-300">{formatPrice(line.unitPrice)} each</p>
